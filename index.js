...
app.get('/jokes/:id', (req, res) => {
  ...
  const foundJoke = find((joke) => joke.id === id);
  res.json(foundJoke);
});
...
app.post('/jokes', (req, res) => {
  ...
  jokes.push(newJoke);
  console.log(jokes.slice(-1)); //this is the go to code to select the last pushed array item
  res.json(newJoke);
});
...
app.patch('/jokes/:id', (req, res) => {
  ...
  const searchIndex = jokes.findIndex((joke) => joke.id === id)
  if(updateType){
    jokes[searchIndex] = updateType;
    res.json(updateType);
  };
  if(updateJokeText){
    jokes[searchIndex] = updateJokeText
    res.json(updateJokeText)
  };
});
...
app.delete('/jokes/:id', (req, res) => {
  ...
  const searchIndex = jokes.findIndex((joke) => joke.id === id);
  if(searchIndex > -1){
    jokes.splice(searchIndex, 1); //removes only one element from the array
    res.sendStatus(200);
  } else {
    res
      .status(404)
